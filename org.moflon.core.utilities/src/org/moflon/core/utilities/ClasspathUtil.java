...
public final class ClasspathUtil
{
   ...
   public static void makeSourceFolder(final IFolder folder) throws CoreException
   {
      ...
      {
         throw new CoreException(new Status(IStatus.ERROR, WorkspaceHelper.getPluginId(ClasspathUtil.class),
               String.format("%s happended while analyzing classpath: %s", e.getClass(), e.getMessage())));
      }
   }

   /**
    * Invokes {@link makeSourceFolder} if {@link RepositoryBuilder#isSourceFolder(IFolder)} returns false for the given folder
