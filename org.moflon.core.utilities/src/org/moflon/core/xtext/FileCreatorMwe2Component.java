...
/**
 * This workflow component creates a file with the at the given path ({@link #setFilePath(String)} with the given content {@link #setFileContent(String)}
 * 
 * Usage example within an .mwe2 file:
...
public class FileCreatorMwe2Component extends AbstractWorkflowComponent2
{
   ...
   /**
    * Creates a file at the configured path ({@link #setFilePath(String)}) using the configured content {@link #setFileContent(String)}
    */
   @Override
   protected void invokeInternal(final WorkflowContext context, final ProgressMonitor monitor, final Issues issues)
   {
      ...
      } catch (final IOException e)
      {
         ...
         issues.addError(this, String.format("Failed to creating file '%s'. Reason: '%s'", this.filePath, e.getMessage()));
      }
   }
