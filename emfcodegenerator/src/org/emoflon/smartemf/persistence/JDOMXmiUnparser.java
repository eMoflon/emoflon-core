...
public class JDOMXmiUnparser {
	...
	public void modelToJDOMTree(final Resource resource, final Document domTree) throws IOException {
		...
		domTree.getRootElement().getAttributes().add(new Attribute(XmiParserUtil.XMI_VERSION_ATR,
				XmiParserUtil.XMI_VERSION, Namespace.getNamespace(XmiParserUtil.XMI_NS, XmiParserUtil.XMI_URI)));
	}

	@SuppressWarnings("unchecked")
	protected Element createDOMTree(EObject root, EObject currentEObject, EReference containment, String id, int idx,
			boolean useSimple) throws IOException {
		...
		if (containment != null) {
			...
			if (metamodel != rootMetamodel) {
				current.getAttributes()
						.add(new Attribute(XmiParserUtil.XSI_TYPE,
								metamodel2NS.get(metamodel).getPrefix() + ":" + currentClass.getName(),
								Namespace.getNamespace(XmiParserUtil.XSI_NS, XmiParserUtil.XSI_URI)));
			} else {
				// If the current object is a sub-type of the edge type then this must be
				// defined in the xmi model
				if (!containment.getEType().equals(currentEObject.eClass())) {
					current.getAttributes()
							.add(new Attribute(XmiParserUtil.XSI_TYPE, ns.getPrefix() + ":" + currentClass.getName(),
									Namespace.getNamespace(XmiParserUtil.XSI_NS, XmiParserUtil.XSI_URI)));
				}
			}
		...
		for (EReference containmentRef : currentClass.getEAllContainments()) {
			...
			for (EObject containee : containees) {
				current.getChildren().add(
						createDOMTree(root, containee, containmentRef, currentId, idIDX, !containmentRef.isMany()));
				idIDX++;
			}
		}

		// Crossrefs
		for (EReference crossRef : currentClass.getEAllReferences().stream().filter(ref -> !ref.isContainment())
				.filter(ref -> {
					...
					if (refObj.eResource() == currentEObject.eResource()) {
						...
						otherPendingRefs.add((crossRefID) -> {
							...
							if (!metamodel2NS.containsKey(otherMetamodel)) {
								Namespace otherNS = Namespace.getNamespace(otherMetamodel.getName(),
										otherMetamodel.getNsURI());
								metamodel2NS.put(otherMetamodel, otherNS);
							}
							if (otherMetamodel != metamodel) {
								pendingCrossRefs.elementIsHref(true, 0, crossRef.getName(),
										metamodel2NS.get(otherMetamodel).getPrefix() + ":" + refObj.eClass().getName());
							} else {
					...
					if (object2ID.containsKey(ref)) {
						...
						if (ref.eResource() == currentEObject.eResource()) {
							...
							otherPendingRefs.add((crossRefID) -> {
								...
								if (otherMetamodel != metamodel) {
									pendingCrossRefs.elementIsHref(true, currentIdx, crossRef.getName(),
											metamodel2NS.get(otherMetamodel).getPrefix() + ":"
													+ ref.eClass().getName());
								} else {
