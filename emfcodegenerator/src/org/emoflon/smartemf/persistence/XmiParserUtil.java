...
public final class XmiParserUtil {
	...
	public static boolean relativeURIsAreEqual(final URI baseUri, final URI uri1, final URI uri2,
	...
	public static String resolveURIToPath(final URI uri, final String workspacePath, boolean existingPathExpected) {
		...
		if (file != null) {
			// Only check if the file exists when it is expected to exist
			if (!existingPathExpected || file.exists()) {
				try {
					return file.getCanonicalPath();
				} catch (IOException e) {
					if (existingPathExpected)
						return null;
				}
			}
		}

		// Otherwise -> fallback solution
		...
		if (!projectFolder.equals(".")) {
			...
			// If the user does not give any information towards a specific folder -> create
			// a path relative to the java exec. path
			try {
				return file.getCanonicalPath();
			} catch (IOException e) {
				return null;
	...
	public static String resolveURIRelativeToBaseURI(final URI baseUri, final URI uri, final String workspacePath) {
		String filePath = resolveURIToPath(uri, workspacePath, true);
		if (filePath != null)
			return filePath;
	...
	public static String pathOfProjectInWorkspace(final String workspacePath, final String projectName)
			throws IOException {
