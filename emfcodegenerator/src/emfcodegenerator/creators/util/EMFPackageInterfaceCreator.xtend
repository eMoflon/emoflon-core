...
class EMFPackageInterfaceCreator extends EMFCodeGenerationClass implements FileCreator{
	...
	def String createSrcCode() {
		...
		public interface «e_pak.get_emf_package_class_name» extends EPackage {
			...
			int «SmartEMFObjectCreator.getLiteral(feature)» = «feature.featureID»;
			«ENDFOR»
			int «SmartEMFObjectCreator.getLiteral(clazz)»_FEATURE_COUNT = «clazz.EStructuralFeatures.size»;
			int «SmartEMFObjectCreator.getLiteral(clazz)»_OPERATION_COUNT = «clazz.EOperations.size»;
