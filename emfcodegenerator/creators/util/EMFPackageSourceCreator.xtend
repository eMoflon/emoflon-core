...
class EMFPackageSourceCreator extends EGenericTypeProcessor implements FileCreator{
	 ...
	 * @param PackageInspector the package for which the file shall be created
	 * @param HashMap<EPackage,PackageInspector> a map containing EPackages as key and an corresponding PackageInspector as value
	 * @param EcoreGenmodelParser the needed ecore-xmi and genmode-xmi wrapper
	 */
	...
	/**
	 * for each direct descendant EClass of an EPackage getter-methods are generated. Each EClass,
	 * its EAttributes, EOperations and EReferences get getters. The generated getters are
	 * automatically added to the methods HashMap and the method_declaration ArrayList.
	 * this method needs to be called before generating EPackageImpl methods
	 * @param EClass the class for which the getters will be generated
	 */
	def private void register_methods_and_data_fields_for_eclass(EClass e_class){
		...
		//var class_name = e_class.name.substring(0, 1).toUpperCase() + e_class.name.substring(1)
		...
		//features have. Thus numerating those is needed.
		var feature_count = 0

		//arraylists to store the precedence of Attributes and Operations
		//it is needed for further ECLass construction
		var ArrayList<ObjectFieldInspector> structural_feature_precedence =
			new ArrayList<ObjectFieldInspector>()
		
		//add object field for EClass
		...
		//for(AbstractObjectFieldInspector inspector : this.e_pak.get_object_field_inspectors_for_class(e_class)){
			...
			var data_type = (inspector.get_inspected_object_type === InspectedObjectType.EATTRIBUTE) ?
						   "EAttribute" : "EReference"

			declaration = "public " + inspector.get_getter_method_declaration_for_the_package_classes()
	...
	/**
	 * creates the data fields and getters for all given EDataType's and registers them for the file
	...
	/**
	 * creates the data fields and getters for all given EEnum's and registers them for the file
	 * which is to be created
	 */
	...
	/**
	 * Generates the EPackage-init() method which is used instead of an public constructor
	 */
	 ...
	 * is the first section of void initializePackageContents()
	 */
	def private ArrayList<String> generate_e_package_init_package_contents__package_init(){
		...
		body.add("setName(" + this.e_pak.get_emf_package_class_name + ".eNAME);")
		body.add("setNsPrefix(" + this.e_pak.get_emf_package_class_name + ".eNS_PREFIX);")
		body.add("setNsURI(" + this.e_pak.get_emf_package_class_name + ".eNS_URI);")
		return body
	}
	...
	def private ArrayList<String> generate_e_package_init_package_contents__create_type_params___for_eclasses(){
		...
		for(e_class : this.e_pak.get_eclasses_which_have_etypeparameters()){
			var e_class_name = e_class.name/*e_class.name.substring(0,1).toLowerCase +
							   e_class.name.substring(1)*/
			for(type_parameter : e_class.ETypeParameters){
				var entry = new StringBuilder("ETypeParameter ")
				var param_var_name = this.e_pak.get_generic_type_to_var_name_map_for_eclass(e_class).get(type_parameter)
		    	entry.append(param_var_name)
		    	entry.append(" = addETypeParameter(")
		    	entry.append(e_class_name)
		    	entry.append("EClass, ")
		    	entry.append('''"«type_parameter.name»");'''.toString)
		    	type_parameter_body.add(entry.toString)
			...
			for(bound : type_param.EBounds) {
				...
				while(bounds_iterator.hasNext){
					...
					if(this.generic_bound_to_var_name_map.keySet.contains(generic_type)){
						...
						get_classifier_command = get_eclassifier_getter_command_for_egenerictype(generic_type)
						//create the generic
						var entry = new StringBuilder()
						entry.append("EGenericType ")
						var var_name = this.generic_bound_to_var_name_map.get(generic_type)
						entry.append(var_name)
						entry.append(" = createEGenericType(")
						entry.append(get_classifier_command)
						entry.append(");")
	...
	def private ArrayList<String> generate_e_package_init_package_contents__init_eclasses(){
		...
		for(e_class : this.e_pak.get_all_eclasses_in_package){
			...
			class_init_command.append(instance_class)
			class_init_command.append(".class, ")
			class_init_command.append('''"«e_class.name»"'''.toString)
			class_init_command.append(", ")
			class_init_command.append(e_class.isAbstract)
			class_init_command.append(", ")
			class_init_command.append(e_class.isInterface)
			class_init_command.append(", ")
			class_init_command.append(is_generated)
			class_init_command.append(");")
			init_block.add(class_init_command.toString)

			body.addAll(
				this.generate_e_package_init_package_contents__init_eclasses___add_super_classes(e_class, e_class_name)
			)
			
			init_block.addAll(
				this.generate_e_package_init_package_contents__init_eclasses___structural_features(e_class)
			)
	...
	def private ArrayList<String> generate_e_package_init_package_contents__init_eenums(){
		...
		for(EEnum eenum : this.e_pak.get_all_eenums_in_package){
			...
			for(entry : eenum.ELiterals){
				body.add('''addEEnumLiteral(«eenum.name»EEnum, «eenum.name».«entry.name.toUpperCase»);'''.toString())
			}
		}
		return body
	}
	 ...
	 * adds to the this.method_declarations and this.methods data-fields
	 */
	...
	/**
	 * generates the method used by EMF to create the EClasses and other EObjects
	 */
	def private void generate_e_package_create_package_contents(){
		...
		//println(this.structural_feature_precedence_map.keySet.equals(this.eoperation_precedence_map.keySet))
		//println(this.structural_feature_precedence_map.keySet.equals(this.e_pak.get_all_eclasses_in_package()))
		...
		for(ecl : this.e_pak.get_all_eclasses_in_package()){
			...
			//create the EClass
			/*
			entry.append(local_class_var_name)
			entry.append(" = ")
			entry.append("this.createEClass(")
			entry.append(emf_to_uppercase(ecl.name))
			entry.append(");")*/
			body.add(
				'''«local_class_var_name» = this.createEClass(«emf_to_uppercase(ecl.name)»);'''.toString
			)

			//create the EAttributes and EReferences, which are not directly inherited
			for(obj_field : this.structural_feature_precedence_map.get(ecl)){
				...
				entry.append(local_class_var_name)
				entry.append(", ")
				entry.append(this.e_pak.get_emf_package_class_name() + ".")
				entry.append(emf_to_uppercase(ecl.name))
				entry.append("__")
				entry.append(emf_to_uppercase(obj_field.get_name))
				entry.append(");")
				body.add(entry.toString)
			}
			...
			for(e_op : this.eoperation_precedence_map.get(ecl)){
				/*entry = new StringBuilder("this.createEOperation(")
				entry.append(local_class_var_name)
				entry.append(", ")
				entry.append(this.e_pak.get_emf_package_class_name() + ".")
				entry.append(emf_to_uppercase(ecl.name))
				entry.append("___")
				entry.append(emf_to_uppercase(e_op.get_name))
				entry.append(");")*/
				body.add(
'''this.createEOperation(«local_class_var_name», «this.e_pak.get_emf_package_class_name()».«emf_to_uppercase(ecl.name)»___«emf_to_uppercase(e_op.get_name)»);'''.toString
		...
		for(e_data_type : this.e_pak.get_all_edata_types_in_package()){
			var entry = new StringBuilder("this.")
			entry.append(e_data_type.name.substring(0, 1).toLowerCase())
			entry.append(e_data_type.name.substring(1))
			entry.append("EDataType = createEDataType(")
			entry.append(emf_to_uppercase(e_data_type.name))
			entry.append(");")
			body.add(entry.toString)
		}
	...
	override write_to_file() {
		...
		for(declaration : this.method_declarations){
			...
			method.append(declaration)
			method.append(" {" + System.lineSeparator)
			
			var iterator = this.methods.get(declaration).iterator
			while(iterator.hasNext){
				method.append(IDENTION + IDENTION)
				method.append(iterator.next)
				method.append(System.lineSeparator)
			}
			method.append(IDENTION+ "}")
			method.append(System.lineSeparator)
			method.append(System.lineSeparator)
		...
		for(declaration : this.object_fields){
			data_fields.append(IDENTION)
			data_fields.append(declaration)
			data_fields.append(System.lineSeparator)
		}data_fields.append(System.lineSeparator)

		package_fw.write(this.package_declaration + System.lineSeparator + System.lineSeparator)
		package_fw.write(import_block.toString)
		package_fw.write(this.class_declaration + " {" + System.lineSeparator + System.lineSeparator)
