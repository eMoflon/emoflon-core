...
/**
 * This {@link InjectionExtractor} builds on an Xtext grammar for injection
 * files
 * 
...
public class XTextInjectionExtractor implements InjectionExtractor {
	...
	/**
	 * Prepares this injection extractor to extract injections from the given
	 * {@link IFolder}. The given {@link GenModel} is used to identify corresponding
	 * {@link GenClass}es and {@link GenOperation}s
	 */
	...
	public IStatus extractInjections() {
		...
		try {
			...
			resultStatus.add(new Status(IStatus.ERROR, WorkspaceHelper.getPluginId(getClass()),
					"Exception during injection extraction", e));
		}
	...
	/**
	 * Returns true of the given file's name starts with
	 * {@link InjectionConstants#IGNORE_FILE_PREFIX}
	 */
	 ...
	 *
	 * We may assume that {@link #shallExtractInjectionsFromFile(IFile)} is true for
	...
	private void extractInjectionFromFile(final IFile injectionFile, final MultiStatus resultStatus) {
		...
		final String filePath = buildInjectionPathDescription(injectionFile, "/").concat(".")
				.concat(WorkspaceHelper.INJECTION_FILE_EXTENSION);
		final GenClass genClass = fqnToGenClassMap.get(fullyQualifiedClassName);
		if (genClass != null) {
			final URI uri = URI.createPlatformResourceURI(injectionFile.getProject().getName() + "/"
					+ injectionRootFolder.getProjectRelativePath() + "/" + filePath, false);
			try {
	...
	/**
	 * Adds an error to the {@link MultiStatus} if the specified class names in both
	 * files mismatch
	 */
	private void validateConnsistentClassName(final IFile injectionFile, final InjectionFile parsedFile,
		...
		if (!classDeclarationName.equals(EclipseResourceUtils.getBasename(injectionFile))) {
			resultStatus.add(new Status(IStatus.WARNING, WorkspaceHelper.getPluginId(getClass()),
					String.format(
							"Basename of injection file '%s' differs from class name '%s' declared inside the file.",
							injectionFile, classDeclarationName)));
		}
	}
	...
	/**
	 * Extracts the fully-qualified name of each {@link GenClass} in the given
	 * {@link GenPackage} and adds it to {@link #fqnToGenClassMap}
	 * 
	 * @param genPackage
	 *            the {@link GenPackage} to process
	 */
	...
	private void reportMissingEClass(final String fullyQualifiedClassName, final IFile injectionFile,
			final MultiStatus resultStatus) {
		final String fullPath = buildInjectionPathDescription(injectionFile, "/").concat(".")
				.concat(WorkspaceHelper.INJECTION_FILE_EXTENSION);
	 ...
	 */
	private void processInjectionFile(final InjectionFile parsedFile, final GenClass genClass,
			final String fullyQualifiedClassName, final IFile injectionFile, final MultiStatus resultStatus) {
		...
		for (final MethodDeclaration methodDeclaration : classDeclaration.getMethodDeclarations()) {
			final EOperation correspondingEOperation = getCorrespondingEOperation(genClass, methodDeclaration,
					resultStatus);
	...
	private String normalizeCodeBody(final String body) {
		...
		return body.substring(0, indexOfEndToken)
				.substring(indexOfBeginToken + InjectionConstants.CODE_BEGIN_TOKEN.length());
	}

	/**
	 * Searches for the EOperation in given EClass, that matches the given name and
	 * parameters
	 *
	 ...
	 * @return The EOperation with the given name and parameters. Guaranteed to be
	 *         non-null.
	 * @throws CoreException
	 */
	private EOperation getCorrespondingEOperation(final GenClass surroundingClass,
			final MethodDeclaration methodDeclaration, final MultiStatus resultStatus) {
	...
	private List<String> extractParameterTypes(final MethodDeclaration methodDeclaration) {
		return methodDeclaration.getParameters().stream().map(declaration -> declaration.getParameterType())
	...
	/**
	 * Checks if the parameters of given EOperation match the given method
	 * parameter.
	 *
	...
	private void reportMissingEOperation(final GenClass surroundingClass, final MethodDeclaration methodDeclaration,
			final IFile injectionFile, final MultiStatus resultStatus) {
		final String fullPath = buildInjectionPathDescription(injectionFile, "/").concat(".")
				.concat(WorkspaceHelper.INJECTION_FILE_EXTENSION);
		final MissingEOperationValidationMessage message = new MissingEOperationValidationMessage(
				methodDeclaration.getMethodName(), extractParameterTypes(methodDeclaration), surroundingClass.getName(),
				fullPath);
		resultStatus.add(message.convertToStatus());
	}
