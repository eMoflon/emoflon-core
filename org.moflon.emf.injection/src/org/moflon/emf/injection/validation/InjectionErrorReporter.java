...
public class InjectionErrorReporter implements ErrorReporter
{
   ...
   {
      final String loggingMessage = "[Injection Validation] " + injectionStatus.getMessage() + ". Project: " + this.project.getName() + ". File: "
            + injectionStatus.getInjectionMessage().getLocation();
   ...
   private void createMarker(final InjectionValidationStatus injectionStatus)
   {
      ...
      try
      {
         final IMarker marker = this.project.getFile(location).createMarker(WorkspaceHelper.INJECTION_PROBLEM_MARKER_ID);
